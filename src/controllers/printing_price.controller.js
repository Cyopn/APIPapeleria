...
export const calculatePrintingPrice = async (req, res, next) => {
    try {
        ...
        const hasAttributes = type === 'bound' || type === 'spiral' || colorModes || paperSizes || ranges || typeof bothSides !== 'undefined' || coverType || bindingType || ringType || anillado;
        if (hasAttributes) {
            const options = { colorModes, paperSizes, ranges, bothSides, sets, type, coverType, bindingType, ringType, anillado };
